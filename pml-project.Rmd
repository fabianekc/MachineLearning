...
save(data_training, file="pml_training.RData")
save(data_testing, file="pml_testing.RData")
```

## Model Fitting
*Random Forest* is used as algorithm to get a small out of sample error. (For performance reason a parallel cluster is setup.)
...
stopCluster(cluster)
save(model, file="pml_model.RData") 
```

## Model Validation

### Training Set Accuracy
```{r training_set_accuracy, cache = TRUE, message=FALSE, warning=FALSE}
print(confusionMatrix(predict(model, data_training), data_training$classe))
```

### Testing Set Accuracy
```{r testing_set_accuracy, cache = TRUE, message=FALSE, warning=FALSE}
print(confusionMatrix(predict(model, data_testing), data_testing$classe))
```
